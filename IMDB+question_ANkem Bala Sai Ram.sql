...
FROM movie
GROUP BY year;
-- 2017 produced the highest number of movies
...
GROUP BY month_num
ORDER BY month_num; 
-- MARCH had the highest numebr of movies
...
limit 1 ;
-- Drama had the highest number of movies produced overall at 4265
...
-- Action genre has the highest duration at 112.88
...
(SELECT production_company,
         ...
         GROUP BY production_company) AS productionCompanyHitMovie
...
SELECT
    name AS actress_name, SUM(total_votes) AS total_votes, COUNT(m.id) AS movie_count,
...
FROM   
		(SELECT DISTINCT title, avg_rating
         ...
         using(movie_id)
         WHERE  
         genre LIKE 'THRILLER') AS thriller_movies;
...
SELECT * 
FROM 
( SELECT genre, year, title AS movie_name, worlwide_gross_income, RANK() OVER(ORDER BY worlwide_gross_income DESC) AS movie_rank
FROM movie AS m 
INNER JOIN genre AS g 
ON m.id=g.movie_id 
WHERE genre IN 
(SELECT genre FROM 
( SELECT genre, COUNT(movie_id) as movie_count, RANK() OVER(ORDER BY COUNT(movie_id) DESC) genre_rank 
FROM genre 
GROUP BY genre 
limit 3 ) AS top3_genre)
) AS ranks
WHERE movie_rank<=5;
...
(SELECT production_company, Count(*) AS movie_count
         ...
         GROUP BY production_company
         ORDER BY movie_count DESC) AS production_company_summary
LIMIT 2; 
...
(
           ...
           AND 
           genre = "Drama"
           GROUP BY   name ) AS actress_summary
           LIMIT 3;
...
(
           ...
           ratings AS r
           ON         
           r.movie_id = m.id ), top_director_summary AS
(
       SELECT *,
...
SELECT name_id AS director_id, NAME AS director_name,Count(movie_id) AS number_of_movies, Round(Avg(date_difference),2) AS avg_inter_movie_days,
         Round(Avg(avg_rating),2) AS avg_rating, Sum(total_votes) AS total_votes, Min(avg_rating) AS min_rating, Max(avg_rating) AS max_rating,
